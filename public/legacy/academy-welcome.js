...
function renderState() {
  ...
  document.getElementById('completionHelp').textContent = state.completed
    ? '100 XP awarded. Meet Atlas is unlocked.'
  ...
  const progress = state.completed ? 50 : 0;
  document.getElementById('progressPercent').textContent = `${progress}%`;
  document.getElementById('progressFill').style.width = `${progress}%`;
  document.getElementById('moduleProgress').style.width = state.completed ? '50%' : '0%';
  ...
  document.getElementById('progressLabel').textContent = state.completed ? 'Welcome Mission 1 complete' : 'Orientation not started';
  document.getElementById('missionCount').textContent = state.completed ? '1 welcome mission complete' : '0 welcome missions complete';
...
document.getElementById('openLesson02').addEventListener('click', () => {
  if (state.completed) window.location.href = 'academy-meet-atlas.html';
});
